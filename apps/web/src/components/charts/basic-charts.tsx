        ...
        <div style={{ height }} className="w-full">
          <svg width="100%" height="100%" viewBox={`0 0 400 ${height}`}>
            ...
            {data.map((item, index) => {
              ...
              const barHeight =
                ((animatedValues[index] || 0) / maxValue) *
                (height - 2 * padding);
              const x =
                padding +
                (320 / data.length) * index +
                (320 / data.length) * 0.1;
...
export function LineChart({
  ...
  const getPoint = (index: number) => {
    ...
    const y =
      height -
      padding -
      ((animatedValues[index] ?? 0 - minValue) / range) *
        (height - 2 * padding);
    return { x, y };
  };
  ...
  const areaPathData =
    pathData +
    ` L ${getPoint(data.length - 1).x} ${height - padding} L ${getPoint(0).x} ${height - padding} Z`;
          ...
          <div style={{ width: size, height: size }} className="relative">
            ...
            {showLabels &&
              data.map((item, index) => {
                ...
                const angle =
                  (animatedAngles[index] ?? 0) + (item.value / total) * 180;
              ...
              {data.map((item, index) => (
                <div key={item.label} className="flex items-center space-x-2">
                  <div
                    ...
                    style={{
                      backgroundColor: getSliceColor(index, item.color),
                    }}
                  />
