...
export class WebhookSignatureService {
     ...
     */
    verifyHMACSignature(payload, signature, timestamp) {
        try {
     ...
     * @returns boolean - true si la firma es válida
     */
    verifyRSASignature(payload, signature, algorithm = 'SHA256withRSA') {
        try {
     ...
     */
    verifyWebhookIntegrity(payload, headers) {
        const result = {
            isValid: false,
            method: 'none',
            errors: [],
     ...
     */
    verifyTimestamp(timestamp, tolerance = 300) {
        try {
...
-----END PUBLIC KEY-----`;
     ...
     */
    validateAEATReference(referencia) {
