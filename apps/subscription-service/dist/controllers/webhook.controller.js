...
class WebhookController {
    ...
    static async handleStripeWebhook(req, res) {
        try {
            ...
            await (0, database_1.withTransaction)(database_2.prisma, async (tx) => {
                // Check if webhook already processed
                const isDuplicate = await (0, database_1.isWebhookDuplicate)(tx, eventId);
                if (isDuplicate) {
     ...
     * Handle AEAT webhook
     */
    static async handleAEATWebhook(req, res) {
        try {
            ...
            await (0, database_1.withTransaction)(database_2.prisma, async (tx) => {
                // Check if webhook already processed
                const isDuplicate = await (0, database_1.isWebhookDuplicate)(tx, numeroJustificante);
                if (isDuplicate) {
                    // Webhook already processed, return success
                    return;
                }
     ...
     */
    static async processPaymentIntentSucceeded(tx, event) {
     ...
     */
    static async processSubscriptionDeleted(tx, event) {
     ...
     */
    static async processInvoicePaymentSucceeded(tx, event) {
     ...
     * Process AEAT notification
     */
    static async processAEATNotification(tx, payload) {
        const numeroJustificante = payload.numeroJustificante;
