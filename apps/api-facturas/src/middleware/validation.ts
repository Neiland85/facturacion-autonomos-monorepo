...
const facturaSchema = Joi.object({
  ...
  lineas: Joi.array().items(
    Joi.object({
      ...
      descuento: Joi.number().min(0).max(100).precision(2).optional().default(0)
    })
  ).optional()
});
...
const facturaUpdateSchema = Joi.object({
  ...
  estado: Joi.string().valid('borrador', 'enviada', 'pagada', 'anulada').optional(),
  lineas: Joi.array().items(
    Joi.object({
...
const clienteSchema = Joi.object({
  ...
  nif: Joi.string().custom((value, helpers) => {
    if (!validarNIF(value)) {
