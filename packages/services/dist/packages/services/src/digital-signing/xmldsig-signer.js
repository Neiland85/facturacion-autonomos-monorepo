...
class XmlDSigSigner {
     ...
     */
    sign(xml, privateKeyPem, certificatePem) {
        try {
    ...
    verify(signedXml) {
        ...
        try {
            ...
            if (signatureNodes.length > 1) {
                errors.push(`Se encontraron ${signatureNodes.length} firmas. Se requiere exactamente 1.`);
     ...
     */
    extractCertificateFromSignature(signedXml) {
        try {
            // Buscar X509Certificate
            const match = /<X509Certificate>([\s\S]*?)<\/X509Certificate>/.exec(signedXml) ||
                /<ds:X509Certificate>([\s\S]*?)<\/ds:X509Certificate>/.exec(signedXml);
            if (!match || !match[1]) {
                return null;
            }
...
exports.XmlDSigSigner = XmlDSigSigner;
