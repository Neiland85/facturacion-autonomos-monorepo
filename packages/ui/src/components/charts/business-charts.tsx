...
export function RevenueChart({
  ...
  const [animatedData, setAnimatedData] = useState(data.map(() => ({ revenue: 0, expenses: 0, profit: 0 })))
        ...
        <div style={{ height }} className="w-full">
          <svg width="100%" height="100%" viewBox={`0 0 400 ${height}`}>
            ...
            {data.map((_, index) => {
              ...
              const current = animatedData[index] ?? { revenue: 0, expenses: 0, profit: 0 }
              const prev = animatedData[index - 1] ?? { revenue: 0, expenses: 0, profit: 0 }
            ...
            {/* Puntos */}
            {data.map((item, index) => {
              const currentData = animatedData[index] ?? { revenue: 0, expenses: 0, profit: 0 }
...
export function CashFlowChart({
  ...
  const [animatedData, setAnimatedData] = useState(data.map(() => ({ inflow: 0, outflow: 0, balance: 0 })))
      ...
      <CardContent>
        <div style={{ height }} className="w-full">
          <svg width="100%" height="100%" viewBox={`0 0 400 ${height}`}>
            ...
            <line
              x1={padding}
              y1={height - padding - ((0 - minValue) / range) * (height - 2 * padding)}
              x2={380}
              y2={height - padding - ((0 - minValue) / range) * (height - 2 * padding)}
              stroke="#9CA3AF"
              strokeWidth="1"
              strokeDasharray="5,5"
            />
            ...
            {data.map((_, index) => {
              if (index === 0) return null
              const current = animatedData[index] ?? { inflow: 0, outflow: 0, balance: 0 }
              const prev = animatedData[index - 1] ?? { inflow: 0, outflow: 0, balance: 0 }
