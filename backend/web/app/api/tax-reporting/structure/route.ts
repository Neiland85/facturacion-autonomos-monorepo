...
ensureServerSide('Tax Reporting API Route');
...
if (!process.env.DATABASE_URL?.startsWith('postgresql://')) {
  throw new Error(
    'DATABASE_URL debe tener el formato correcto: postgresql://user:password@host.tld/dbname. Verifica tu archivo .env.'
  );
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const processedInvoices: OCRInvoiceData[] = invoicesResult.map((row: any) =>
      JSON.parse(row.invoice_data)
    );
    ...
    processedInvoices.forEach(invoice => {
      ...
      const deductibilityFactor =
        (invoice.deductibilityPercentage || 100) / 100;
      ...
      if (invoice.taxCategory.type === 'expense') {
        ...
        if (expensesByCategory[category]) {
          ...
          expensesByCategory[category].vatDeductible +=
            vatAmount * deductibilityFactor;
    ...
    const taxReportingStructure: TaxReportingStructure = {
      ...
      validationSummary: {
        ...
        successfulExtractions: processedInvoices.filter(
          inv => inv.confidence >= 0.7
        ).length,
        requiresManualReview: processedInvoices.filter(
          inv => inv.confidence < 0.7
        ).length,
        avgConfidenceScore:
          processedInvoices.length > 0
            ? processedInvoices.reduce((sum, inv) => sum + inv.confidence, 0) /
