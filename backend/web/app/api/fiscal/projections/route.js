...
function calculateProjection(historicalData) {
...
function generateProjectionAdvice(historicalData, projection) {
    const advice = [];
...
async function POST(request) {
    try {
        ...
        if (!historicalData || !Array.isArray(historicalData) || historicalData.length === 0) {
            return server_1.NextResponse.json({ message: "Valid 'historicalData' array is required in the request body." }, { status: 400 });
        }
        ...
        for (const item of historicalData) {
            if (typeof item.quarter !== "string" ||
                ...
                return server_1.NextResponse.json({ message: "Invalid item structure in 'historicalData'." }, { status: 400 });
            }
        }
        ...
        const response = { projection, advice };
        ...
        return server_1.NextResponse.json({ message: errorMessage }, { status: 500 });
    }
}
