...
import { Loader2, AlertTriangle, CheckCircle, FileText, Download, ArrowRight, ArrowLeft, Info } from "lucide-react"
import type {
  TaxWorkflowData,
...
const runAiAnalysis = (activity: QuarterlyActivityData, m303?: Modelo303Data, m130?: Modelo130Data): AIWarning[] => {
  const warnings: AIWarning[] = []
  if (activity.totalExpenses / (activity.totalIncome || 1) > 0.75) {
    warnings.push({
      id: "exp_ratio",
      message:
        "Ratio de gastos elevado (superior al 75% de los ingresos). Esto podría llamar la atención de la Agencia Tributaria.",
      severity: "warning",
  ...
  if (
    ...
    warnings.push({
      ...
      message:
        "El Modelo 303 resulta en una cantidad a devolver significativa. Verifica que todas las cuotas soportadas son correctas.",
      severity: "warning",
  ...
  if (
    ...
    warnings.push({
      id: "m130_bajo",
      message:
        "El pago fraccionado del Modelo 130 es muy bajo o cero. Confirma que los ingresos y gastos están correctamente reportados.",
      severity: "info",
...
const calculateModelo303 = (activity: QuarterlyActivityData): Modelo303Data => {
...
const calculateModelo130 = (activity: QuarterlyActivityData): Modelo130Data => {
...
const mockFilingAdvice: FilingAdviceItem[] = [
  {
    id: "adv_deadline",
    title: "Plazo de Presentación",
    description:
      "El plazo para presentar los modelos trimestrales finaliza el día 20 del mes siguiente al trimestre (Abril, Julio, Octubre, Enero). ¡No lo dejes para el último día!",
    category: "deadline",
  },
  {
    id: "adv_docs",
    ...
    description:
      "Ten a mano todas tus facturas emitidas y recibidas, extractos bancarios y justificantes de gastos. Una buena organización es clave.",
  ...
  },
  {
    ...
    description:
      "Revisa si puedes aplicar la reducción por inicio de actividad (si aplica) o si tienes retenciones soportadas que puedas deducir.",
    category: "deduction",
  },
  {
    id: "adv_vat_check",
    ...
    description:
      "Asegúrate de que el IVA soportado en tus gastos es deducible y corresponde a actividades afectas. Comprueba los tipos de IVA aplicados.",
    category: "deduction",
  },
  {
    ...
    description:
      "Necesitarás tu certificado digital o Cl@ve PIN para la presentación telemática en la sede electrónica de la AEAT.",
    category: "general",
  },
]
// --- End Mock Data & API ---
...
const TaxFormFieldDisplay: React.FC<{ field: TaxFormField; highlight?: boolean }> = ({ field, highlight }) => (
  <div className={`p-2 border-b dark:border-slate-700 ${highlight ? "bg-primary/10 dark:bg-primary/20" : ""}`}>
    <div className="flex justify-between items-center text-sm">
      <span className="text-muted-foreground flex items-center">
        {field.label}
        {field.tooltip && <Info size={12} className="ml-1 text-gray-400 cursor-help" title={field.tooltip} />}
      </span>
      <span
        className={`font-semibold ${
          field.validationStatus === "ok"
...
const AIWarningCard: React.FC<{ warning: AIWarning; onFieldHighlight: (fieldIds: string[] | undefined) => void }> = ({
  warning,
  onFieldHighlight,
}) => (
  <Card
    ...
    onMouseEnter={() => onFieldHighlight(warning.relatedFieldIds)}
    onMouseLeave={() => onFieldHighlight(undefined)}
  >
    <CardContent className="p-3">
      <div className="flex items-start gap-3">
        {warning.severity === "critical" && <AlertTriangle className="h-5 w-5 text-red-500 flex-shrink-0" />}
        {warning.severity === "warning" && <AlertTriangle className="h-5 w-5 text-yellow-500 flex-shrink-0" />}
        {warning.severity === "info" && <Info className="h-5 w-5 text-blue-500 flex-shrink-0" />}
        <div>
          <p
...
export default function TaxFilingWorkflow({ triggerQuarter }: { triggerQuarter: string }) {
  const [currentStep, setCurrentStep] = useState(1)
  const [isLoading, setIsLoading] = useState(true)
  const [workflowData, setWorkflowData] = useState<TaxWorkflowData | null>(null)
  const [highlightedFieldIds, setHighlightedFieldIds] = useState<string[] | undefined>(undefined)

  useEffect(() => {
    const fetchDataAndProcess = async () => {
      setIsLoading(true)
      const activitySummary: QuarterlyActivityData = await simulateApiCall(
        {
  ...
  const handleCalculateForms = async () => {
    if (!workflowData || !workflowData.activitySummary) return
    setIsLoading(true)
    ...
    setWorkflowData((prev) =>
      prev ? { ...prev, modelo303: m303, modelo130: m130, aiWarnings: updatedAiWarnings } : null,
    )
    setIsLoading(false)
  }
    ...
    if (isLoading && !workflowData)
      return (
        <div className="flex flex-col justify-center items-center p-10 min-h-[300px]">
          <Loader2 className="h-8 w-8 animate-spin text-primary" /> <span className="ml-2 mt-2">Cargando datos...</span>
        </div>
      )
    if (!workflowData) return <p className="text-center text-red-500 p-10">Error al cargar los datos.</p>
    ...
    switch (currentStep) {
      case 1: // Activity Summary
        return (
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}>
            <CardHeader>
              <CardTitle>Resumen de Actividad Fiscal - {workflowData.activitySummary.period}</CardTitle>
              <CardDescription>Verifica tu actividad del trimestre antes de proceder.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
                <Card className="p-4 shadow-sm dark:bg-slate-800">
                ...
                <Card className="p-4 shadow-sm dark:bg-slate-800">
                  ...
                  <p
                    className={`text-2xl font-bold ${workflowData.activitySummary.netProfit >= 0 ? "text-primary" : "text-orange-500 dark:text-orange-400"}`}
                  >
              ...
              {workflowData.aiWarnings.filter((w) => !w.relatedFieldIds || w.relatedFieldIds.length === 0).length >
                0 && <Separator className="my-4 dark:bg-slate-700" />}
              {workflowData.aiWarnings
                .filter((w) => !w.relatedFieldIds || w.relatedFieldIds.length === 0)
                .map((warning) => (
                  <AIWarningCard key={warning.id} warning={warning} onFieldHighlight={setHighlightedFieldIds} />
                ))}
            </CardContent>
          </motion.div>
        )
      case 2: // Tax Form Calculation
        ...
        return (
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}>
            <CardHeader>
              <CardTitle>Modelos Fiscales Calculados - {workflowData.activitySummary.period}</CardTitle>
              <CardDescription>Revisa los borradores de los modelos 303 (IVA) y 130 (IRPF).</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Modelo 303 */}
                <Card className="shadow-md dark:bg-slate-800">
                  <CardHeader className="bg-muted/30 dark:bg-slate-700/50">
                  ...
                  <CardContent className="p-0">
                    <div className="max-h-[300px] overflow-y-auto">
                      ...
                      {workflowData.modelo303.regimenGeneral.baseImponibleRepercutido.map((f) => (
                        <TaxFormFieldDisplay key={f.id} field={f} highlight={highlightedFieldIds?.includes(f.id)} />
                      ))}
                      {workflowData.modelo303.regimenGeneral.cuotaRepercutida.map((f) => (
                        <TaxFormFieldDisplay key={f.id} field={f} highlight={highlightedFieldIds?.includes(f.id)} />
                      ))}
                      {workflowData.modelo303.regimenGeneral.baseImponibleSoportado.map((f) => (
                        <TaxFormFieldDisplay key={f.id} field={f} highlight={highlightedFieldIds?.includes(f.id)} />
                      ))}
                      {workflowData.modelo303.regimenGeneral.cuotaSoportada.map((f) => (
                        <TaxFormFieldDisplay key={f.id} field={f} highlight={highlightedFieldIds?.includes(f.id)} />
                      ))}
                      <h4 className="text-sm font-semibold p-3 bg-slate-100 dark:bg-slate-700 sticky top-0 z-10">
                        Resultados
                      ...
                      <TaxFormFieldDisplay
                        field={workflowData.modelo303.totalCuotaDevengada}
                        highlight={highlightedFieldIds?.includes(workflowData.modelo303.totalCuotaDevengada.id)}
                      />
                      <TaxFormFieldDisplay
                        field={workflowData.modelo303.totalCuotasDeducibles}
                        highlight={highlightedFieldIds?.includes(workflowData.modelo303.totalCuotasDeducibles.id)}
                      />
                      <TaxFormFieldDisplay
                        field={workflowData.modelo303.resultadoRegimenGeneral}
                        highlight={highlightedFieldIds?.includes(workflowData.modelo303.resultadoRegimenGeneral.id)}
                      />
                    </div>
                    <CardFooter className="p-3 bg-primary text-primary-foreground mt-0">
                ...
                <Card className="shadow-md dark:bg-slate-800">
                  ...
                  <CardContent className="p-0">
                    <div className="max-h-[300px] overflow-y-auto">
                      ...
                      {Object.values(workflowData.modelo130.rendimientosActividad).map((f) => (
                        <TaxFormFieldDisplay key={f.id} field={f} highlight={highlightedFieldIds?.includes(f.id)} />
                      ))}
                      <h4 className="text-sm font-semibold p-3 bg-slate-100 dark:bg-slate-700 sticky top-0 z-10">
                        Liquidación
                      </h4>
                      {Object.values(workflowData.modelo130.liquidación).map((f) => (
                        <TaxFormFieldDisplay key={f.id} field={f} highlight={highlightedFieldIds?.includes(f.id)} />
                      ))}
                    </div>
                    <CardFooter className="p-3 bg-primary text-primary-foreground mt-0">
              ...
              {workflowData.aiWarnings.filter((w) => w.relatedFieldIds && w.relatedFieldIds.length > 0).length > 0 && (
      ...
      case 3: // Filing Advice & PDF Generation
        return (
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}>
            <CardHeader>
              <CardTitle>Consejos y Descarga de Modelos</CardTitle>
              <CardDescription>Recomendaciones finales y acceso a tus modelos listos para presentar.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div>
                <h3 className="text-md font-semibold mb-2 text-primary">Consejos para la Presentación:</h3>
                <ul className="space-y-3">
                  {workflowData.filingAdvice.map((advice) => (
                    <li
                      key={advice.id}
                      className="p-3 border rounded-lg shadow-sm bg-muted/30 dark:bg-slate-800 dark:border-slate-700"
                    >
              ...
              <div>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <Button
                    ...
                    onClick={() => alert("Simulación: Descargando PDF Modelo 303...")}
                  >
                    <Download size={16} className="mr-2" /> Generar PDF Modelo 303
                  </Button>
                  <Button
                    className="w-full sm:w-auto bg-primary hover:bg-primary/90 text-primary-foreground"
                    onClick={() => alert("Simulación: Descargando PDF Modelo 130...")}
                  >
                    <Download size={16} className="mr-2" /> Generar PDF Modelo 130
                  </Button>
                </div>
                <p className="text-xs text-muted-foreground mt-3">
                  <CheckCircle size={14} className="inline mr-1 text-green-500 dark:text-green-400" />
  ...
  return (
    <Card className="w-full max-w-3xl mx-auto shadow-xl dark:bg-slate-800/50 dark:border-slate-700">
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="min-h-[350px]" // Ensure consistent height during transitions
        >
          {renderStepContent()}
        </motion.div>
      </AnimatePresence>
      ...
      <CardFooter className="flex justify-between p-4 border-t dark:border-slate-700">
        ...
        {currentStep < steps.length ? (
          ...
          >
            {isLoading && currentStep === 1 ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null}
            Siguiente <ArrowRight size={16} className="ml-1" />
          </Button>
        ) : (
          <Button
            onClick={() => alert("Proceso completado. ¡Recuerda presentar tus impuestos!")}
